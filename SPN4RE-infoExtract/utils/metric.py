def metric(pred, gold):
    # gold: {0: [(三元组1), (三元组2), ...], 1: [...]},其中(三元组)的数据形式与pred里10种的每一种相同
    # pred: # {0: [{"pred_rel": int, "head_start_index": int, ...}, {}, ...省略10个字典], 1: []} 5元组
    ...
    for sent_idx in pred:
        ...
        for ele in prediction: # 拿出当前句子预测的可能三元组
            if ele in gold[sent_idx]: # 判断预测的三元组是否在gold里
                right_num += 1
                pred_correct_num += 1
            if ele[0] in [e[0] for e in gold[sent_idx]]: # 判断当前句子预测的关系
                rel_num += 1
...
def num_metric(pred, gold):
    ...
    for sent_idx in gold: # for循环取出key
        if len(gold[sent_idx]) == 1: # 一个三元组
            test_1.append(sent_idx)
        elif len(gold[sent_idx]) == 2: # 两个三元组
...
def overlap_metric(pred, gold):
    ...
    for sent_idx in gold:
        triplets = gold[sent_idx] # 当前句子的三元组列表
...
def is_multi_label(triplets): # 一个实体对对应多种关系
    if is_normal_triplet(triplets):
        return False
...
def get_key_val(dict_1, list_1):
    dict_2 = dict()
    for ele in list_1:
        dict_2.update({ele: dict_1[ele]}) # {sent index：三元组}
    return dict_2
